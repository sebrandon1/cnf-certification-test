...
func populateXMLFromClaim(c claim.Claim, startTime, endTime time.Time) TestSuitesXML {
	...
	xmlOutput.Time = strconv.FormatFloat(float64(endTime.Sub(startTime).Seconds()), 'f', 3, 64)
	...
	xmlOutput.Testsuite.Time = strconv.FormatFloat(float64(endTime.Sub(startTime).Seconds()), 'f', 3, 64)
